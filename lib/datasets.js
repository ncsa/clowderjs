...
exports.listFiles = function(auth, datasetID, callback) {
  var options = {
    auth: {
      user: auth.authorId,
      pass: auth.password
  ...
  request
    .get(auth.api + '/datasets/' + datasetID + '/listFiles', options, function(err, response, responseBody){
      if (err) callback(err);
      try {
 ...
 * @param callback
 */
exports.download = function(auth, datasetID, callback) {
  var options = {
    auth: {
      user: auth.authorId,
      pass: auth.password
  ...
  request
    .get(auth.api + '/datasets/' + datasetID + '/download', options, function(err, response, responseBody){
      if (err) callback(err);
      try {
        callback(JSON.parse(responseBody));
      } catch(e) {
